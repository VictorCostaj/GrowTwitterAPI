...
class AuthController {
  public async create(req: Request, res: Response) {
    ...
    const usuario = await usuarioService.PuxaNomeSenha(username, senha);
  ...
  public async delete(req: Request, res: Response) {
    ...
    const usuario = await usuarioService.PuxaNomeSenha(token as string);
    ...
    const response: ResponseDto = {
      code: 404,
      message:
        "Acesso não Realizado! (Não Informado o Token ou invalido no header!)",
    };
